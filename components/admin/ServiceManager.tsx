...
const ServiceManager: React.FC = () => {
    ...
    const fetchServices = useCallback(async () => {
        ...
        try {
            const filters = auth?.user?.role === 'regional_admin' && auth.user.managedProvince
                ...
                : {};
            const services = await getAllServicesForAdmin(filters);
    ...
    const filteredServices = useMemo(() => {
        return allServices.filter(service => {
            ...
            // Si el admin es general, el filtro de provincia funciona normalmente.
            // Si el admin es regional, su vista ya está pre-filtrada por fetchServices, pero este filtro adicional no hace daño.
            const provinceMatch = provinceFilter === '' || service.province === provinceFilter;

            return statusMatch && searchMatch && provinceMatch;
        });
    }, [allServices, statusFilter, searchQuery, provinceFilter]);
