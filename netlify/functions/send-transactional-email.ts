
import { Handler } from "@netlify/functions";
import formData from 'form-data';
import Mailgun from 'mailgun.js';

// Tipos de datos que esperamos recibir en el body de la petición
interface EmailRequest {
    recipientEmail: string;
    recipientName: string;
    emailType: 'submission-confirmation' | 'submission-approved' | 'submission-rejected';
    item: {
        name: string;
        type: 'Servicio' | 'Evento';
        rejectionReason?: string;
        id: string;
    };
}

// Inicialización de Mailgun
const mailgun = new Mailgun(formData);
const mg = mailgun.client({
  username: 'api',
  key: process.env.MAILGUN_API_KEY || '',
});

const handler: Handler = async (event, _context) => {
    if (event.httpMethod !== 'POST') {
        return { statusCode: 405, body: 'Method Not Allowed' };
    }

    if (!event.body) {
        return { statusCode: 400, body: 'Bad Request: Missing body' };
    }

    try {
        const requestBody = JSON.parse(event.body) as EmailRequest;
        const { recipientEmail, recipientName, emailType, item } = requestBody;

        const baseUrl = 'https://www.ecuadorencanada.com';
        let subject = '';
        let html = '';

        // Construir el correo según el tipo
        switch (emailType) {
            case 'submission-confirmation':
                subject = `Confirmación de Envío`;
                html = composeEmail(
                    `Confirmación de Envío`,
                    `¡Hola, ${recipientName}!`,
                    `Gracias por contribuir a la comunidad. Hemos recibido tu ${item.type} "${item.name}" y lo estamos revisando. Te notificaremos de nuevo cuando sea aprobado.`, 
                    {
                        text: 'Ver mis envíos', // Placeholder link
                        url: `${baseUrl}/dashboard`
                    }
                );
                break;

            case 'submission-approved':
                subject = `¡Tu ${item.type} ha sido aprobado!`;
                html = composeEmail(
                    `¡Buenas Noticias!`,
                    `¡Felicidades, ${recipientName}!`,
                    `Tu ${item.type} "${item.name}" ha sido aprobado y ya está visible para toda la comunidad. ¡Gracias por tu increíble aporte!`, 
                    {
                        text: `Ver tu ${item.type}`,
                        url: `${baseUrl}/${item.type.toLowerCase()}s/${item.id}`
                    }
                );
                break;

            case 'submission-rejected':
                subject = `Actualización sobre tu ${item.type}`;
                html = composeEmail(
                    `Actualización sobre tu envío`,
                    `Hola, ${recipientName}`,
                    `Hemos revisado tu ${item.type} "${item.name}" y requiere algunos cambios antes de poder publicarlo. Motivo: ${item.rejectionReason || 'No se especificó un motivo.'}. Por favor, edita tu envío y vuelve a mandarlo. ¡Gracias!`, 
                    {
                        text: 'Editar mi envío', // Placeholder link
                        url: `${baseUrl}/dashboard`
                    }
                );
                break;

            default:
                return { statusCode: 400, body: 'Bad Request: Invalid emailType' };
        }

        // Enviar el correo
        await mg.messages.create(process.env.MAILGUN_DOMAIN || '', {
            from: process.env.MAILGUN_FROM_EMAIL,
            to: recipientEmail,
            bcc: ['christian.alexis.banda@gmail.com', 'diegovinuezaleon@gmail.com'],
            subject: subject,
            html: html,
        });

        return { statusCode: 200, body: 'Email sent successfully' };

    } catch (error) {
        console.error("Error processing email request:", error);
        return { statusCode: 500, body: `Internal Server Error: ${error.message}` };
    }
};

// Plantilla de correo base (reutiliza el estilo que ya creamos)
function composeEmail(title: string, heading: string, text: string, cta: { text: string, url: string }): string {
    const colors = { primary: '#003366', secondary: '#FFD700', background: '#f4f7f6', cardBackground: '#ffffff', text: '#333333', lightText: '#666666', white: '#ffffff' };
    const baseUrl = 'https://www.ecuadorencanada.com';
    const logoUrl = `${baseUrl}/ecuanada.png`;

    return `
    <!DOCTYPE html>
    <html lang="es">
    <head><meta charset="UTF-8"></head>
    <body style="margin: 0; padding: 0; background-color: ${colors.background}; font-family: Arial, sans-serif;">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: ${colors.background};">
            <tr>
                <td align="center">
                    <table width="600" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; margin: 20px auto;">
                        <tr><td align="center" style="padding: 20px 0;"><img src="${logoUrl}" alt="Conexión EC-CA Logo" width="180" style="display: block;"></td></tr>
                        <tr>
                            <td style="background-color: ${colors.cardBackground}; padding: 40px 30px; border-radius: 8px;">
                                <h1 style="color: ${colors.primary}; font-size: 28px; margin-top: 0; margin-bottom: 15px;">${heading}</h1>
                                <p style="color: ${colors.text}; font-size: 16px; line-height: 1.6;">${text}</p>
                                <a href="${cta.url}" target="_blank" style="background-color: ${colors.primary}; color: ${colors.white}; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-weight: bold; display: inline-block; margin-top: 20px;">
                                    ${cta.text}
                                </a>
                            </td>
                        </tr>
                        <tr><td align="center" style="padding: 20px 0; font-size: 12px; color: ${colors.lightText};"><p>&copy; ${new Date().getFullYear()} Conexión EC-CA</p></td></tr>
                    </table>
                </td>
            </tr>
        </table>
    </body>
    </html>`;
}

export { handler };
